...
output_directory = './PO3/'  # 결과 파일을 저장할 디렉토리 경로
...
for csv_file in files_to_process:
    ...
    # EEG 데이터 (여기에서는 PO3 채널만 사용)
    PO3 = df.iloc[:, 4].values  # PO3 채널이 다섯 번째 열에 위치한다고 가정 (열 인덱스는 4)

    # 각 주파수 대역에 대해 필터 적용
    filtered_signals_PO3 = {}
    for band, (lowcut, highcut) in frequency_bands.items():
        filtered_signals_PO3[band] = butter_bandpass_filter(PO3, lowcut, highcut, fs, order=N)

    # 필터링된 데이터프레임 생성
    df_filtered = pd.DataFrame(filtered_signals_PO3)
    ...
    condition = parts[2]
    output_file = os.path.join(output_directory, f'sub_{subject}_{condition}_PO3_full.csv')
