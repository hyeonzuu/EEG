...
output_directory = './FCZ/'  # 결과 파일을 저장할 디렉토리 경로
...
for csv_file in files_to_process:
    ...
    # EEG 데이터 (여기에서는 FCZ 채널만 사용)
    FCZ = df.iloc[:, 6].values  # FCZ 채널이 일곱 번째 열에 위치한다고 가정 (열 인덱스는 6)

    # 각 주파수 대역에 대해 필터 적용
    filtered_signals_FCZ = {}
    for band, (lowcut, highcut) in frequency_bands.items():
        filtered_signals_FCZ[band] = butter_bandpass_filter(FCZ, lowcut, highcut, fs, order=N)

    # 필터링된 데이터프레임 생성
    df_filtered = pd.DataFrame(filtered_signals_FCZ)
    ...
    condition = parts[2]
    output_file = os.path.join(output_directory, f'sub_{subject}_{condition}_FCZ_full.csv')
